...
auto ImportDialog::run(string_vector locations) -> void {
  ...
  for(auto& location : locations) {
    ...
    if(!daedalus.import(location)) {
      errors.append(string{"[", name, "] ", daedalus.error()});
    }
  }
  setVisible(false);
  ...
  if(errors) {
    ...
    if(MessageDialog().setTitle("daedalus").setText(message).question() == "Yes") {
      errorDialog->show(errors.merge("\n"));
    } else {
      scanDialog->show();
    }
  } else {
    MessageDialog().setTitle("daedalus").setText("Import completed successfully.").information();
    scanDialog->show();
  }
}
